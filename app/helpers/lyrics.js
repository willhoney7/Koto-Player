lyrics = ({
	...
	searchForLyrics: function(song_, callBackFunc){
		var song = Object.clone(song_);
		//Search first
		g.AjaxRequest.request(this.base_url + "track.search", {
			method: "get",
			parameters: {
			...
			onSuccess: function(transport){
				...
				if(response.message.header.status_code === 200){
					if(response.message.header.available > 0){
						this.getLyrics(response.message.body.track_list[0].track.track_id, callBackFunc);
					}else {
						var regex = /(\(.*?\)|\[.*?\])/g; //remove extra stuff on titles..
						if(regex.test(song.title)){
							song.title = song.title.replace(regex, "");
							this.searchForLyrics({title: song.title, artist: song.artist}, callBackFunc);
						} else {
							callBackFunc({error: "Lyrics Not Found"});		
						}						
					}
				} else {
					m.debugErr("Request Failed. Error Code #" + response.message.header.status_code);
					callBackFunc({error: "Lyrics Request Failed"});
				}
	...
	getLyrics: function(id, callBackFunc){
		g.AjaxRequest.request(this.base_url + "track.lyrics.get", {
			method: "get",
			parameters: {
				"apikey": lastfm.utf8_encode(this.api_key),
		...
		g.AjaxRequest.request("http://api.lyricsfly.com/api/api.php",{
			...
			onSuccess: function(transport){
				...
				if(status == 200 || status == 300)
					//callBackFunc({lyrics: this.parseLyrics(xml.getElementsByTagName("tx")[0].childNodes[0].nodeValue)});
					callBackFunc({lyrics: (xml.getElementsByTagName("tx")[0].childNodes[0].nodeValue).replace(/(\[br\]\[br\]|\[br\]\s\[br\])/g, "").replace(/\[br\]/g, "<br\/>")});
				else if (status == 204)
					callBackFunc({error: "Lyrics Not Found"});
				else {
					sg = xml.getElementsByTagName("sg")[0].childNodes[0].nodeValue;
					callBackFunc({error: "Error: " + sg});
				}
			}.bind(this),
			onFailure: function(transport){
				callBackFunc({error: "Lyrics Request Failed"});		
